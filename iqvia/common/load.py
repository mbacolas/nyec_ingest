...
def load_patient(spark: SparkSession, load_path: str, schema: StructType, batch_id: str):
    return load_df(spark, load_path, schema) \
...
def load_procedure(spark: SparkSession, load_path: str, schema: StructType):
    return load_df(spark, load_path, schema)


def load_procedure_modifier1(spark: SparkSession, load_path: str, schema: StructType):
    return load_df(spark, load_path, schema) \
        .select(col('PRC_MODR_CD').alias('PRC1_MODR_CD'), col('PRC_MODR_DESC').alias('PRC1_MODR_DESC'))


def load_procedure_modifier2(spark: SparkSession, load_path: str, schema: StructType):
    return load_df(spark, load_path, schema) \
...
def load_procedure_modifier4(spark: SparkSession, load_path: str, schema: StructType):
    return load_df(spark, load_path, schema) \
        .select(col('PRC_MODR_CD').alias('PRC4_MODR_CD'), col('PRC_MODR_DESC').alias('PRC4_MODR_DESC'))


def load_diagnosis(spark: SparkSession, load_path: str, schema: StructType):
    return load_df(spark, load_path, schema)


def load_drug(spark: SparkSession, load_path: str, schema: StructType):
    return load_df(spark, load_path, schema)


def load_provider(spark: SparkSession, load_path: str, schema: StructType):
    return load_df(spark, load_path, schema)

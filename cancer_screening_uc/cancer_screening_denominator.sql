create table curated.denom as (
    with denom_inclusion as (
    select
        p.source_org_oid,
        p.source_consumer_id
    from
        curated.patient p
    INNER JOIN
    ...
    where
        datediff(year, p.dob, to_date('2021-12-01', 'YYYY-MM-DD')) between 50 and 75
--         datediff(year, p.dob, current_date) between 50 and 75
    ...
    ON proc.code=ccc.code and proc.code_system=ccc.code_system
    where
        ccc.test_name = 'Denom Exclusion' and ccc.code_system in ('CPT', 'HCPCS')
    UNION
    select
        source_org_oid,
        source_consumer_id
    ...
    where
        ccc.test_name = 'Denom Exclusion' and ccc.code_system = 'ICD10'
    UNION
    select
        p.source_org_oid,
        p.source_consumer_id
...
--         and
    ...
    select
        distinct d_in.source_org_oid, d_in.source_consumer_id
    from
...
select
    count(n.source_consumer_id) -- 1 295 175
from
curated.numerator_criteria n -- 1 942 384
inner join
curated.denom d -- 6 458 213
on n.source_consumer_id=d.source_consumer_id;
